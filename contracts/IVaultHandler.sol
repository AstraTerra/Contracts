...
/**
 * @title HMKT Vault Handler Abstract Contract
 * @author AstraTerra.Finance
 * @notice Contract in charge of handling the HMKT Token and stake
 */
  ...
  IERC165
{
  ...
  /**
   * @notice Vault object created to manage the mint and burns of HMKT tokens
   * @param Id, unique identifier of the vault
   * @param Collateral, current collateral on vault
   * @param Debt, current amount of HMKT tokens minted
  ...
  /// @notice HMKT Token Address
  HMKT public immutable HMKTToken;

  /// @notice Total Market Cap/USD Oracle Address
  ChainlinkOracle public immutable HMKTOracle;
  ...
  /// @notice Value used as divisor with the total market cap, just like the S&P 500 or any major financial index would to define the final HMKT token price
  uint256 public divisor;

  /// @notice Minimum ratio required to prevent liquidation of vault
  uint256 public ratio;

  /// @notice Fee percentage of the total amount to burn charged on ETH when burning HMKT Tokens
  uint256 public burnFee;

  /// @notice Penalty charged to vault owner when a vault is liquidated, this value goes to the liquidator
  uint256 public liquidationPenalty;

  /// @notice Minimum amount of HMKT an user can mint
  uint256 public minimumHMKT = 0;
  ...
  /// @notice An event emitted when the minimum required HMKT is updated
  ...
  constructor(
    ...
    HMKTOracle = ChainlinkOracle(_HMKTOracle);
    ...
    HMKTToken = _HMKTAddress;
    treasury = _treasury;
    minimumHMKT = _minimumHMKT;
  ...
  /**
   * @notice Sets the burn fee percentage an user pays when burning HMKT tokens
   * @param _burnFee uint
   * @dev Only owner can call it
   */
  ...
  /**
   * @notice Sets the minimum HMKT a vault owner can mint
   * @param _minimumHMKT uint
   * @dev Only owner can call it
   * @dev this is mostly used on hard mode vaults with low collateral ratio vaults
   */
  function setMinimumHMKT(uint256 _minimumHMKT) external virtual onlyOwner {
    minimumHMKT = _minimumHMKT;
    emit NewMinimumHMKT(msg.sender, _minimumHMKT);
  }
  ...
  /**
   * @notice Allows users to remove collateral currently not being used to generate HMKT tokens from their vaults
  ...
  /**
   * @notice Uses collateral to generate debt on HMKT Tokens which are minted and assigned to caller
    ...
    notZero(_amount)
  {
    ...
    require(
      vault.Debt >= minimumHMKT,
      "VaultHandler::mint: mint amount less than required"
    );

    HMKTToken.mint(msg.sender, _amount);
    emit TokensMinted(msg.sender, vault.Id, _amount);
  }

  /**
   * @notice Pays the debt of HMKT tokens resulting them on burn, this releases collateral up to minimum vault ratio
  ...
  /**
   * @notice Allow users to burn HMKT tokens to liquidate vaults with vault collateral ratio under the minimum ratio, the liquidator receives the staked collateral of the liquidated vault at a premium
   * @param _vaultId to liquidate
   * @param _maxHMKT max amount of HMKT the liquidator is willing to pay to liquidate vault
   * @dev Resulting ratio must be above or equal minimum ratio
   * @dev A fee of exactly burnFee must be sent as value on ETH
   * @dev The fee goes to the treasury contract
   */
  function liquidateVault(uint256 _vaultId, uint256 _maxHMKT)
    external
    payable
    nonReentrant
    whenNotPaused
    whenNotDisabled(FunctionChoices.LiquidateVault)
  {
    ...
    uint256 requiredHMKT = requiredLiquidationHMKT(vault.Id);
    require(
      _maxHMKT >= requiredHMKT,
      "VaultHandler::liquidateVault: liquidation amount different than required"
    );

    uint256 fee = getFee(requiredHMKT);
    require(
      msg.value >= fee,
    ...
    //send back ETH above fee
    safeTransferETH(msg.sender, msg.value.sub(fee));
    emit VaultLiquidated(vault.Id, msg.sender, requiredHMKT, reward);
  }
  ...
  /**
   * @notice Returns the price of the HMKT token
   * @return price of the HMKT Token
   * @dev HMKT token is 18 decimals
   ...
   * d = Divisor
   */
  function HMKTPrice() public view virtual returns (uint256 price) {
    uint256 totalMarketPrice = getOraclePrice(HMKTOracle);
    price = totalMarketPrice.div(divisor);
  }

  /**
   * @notice Returns the minimal required collateral to mint HMKT token
   * @param _amount uint amount to mint
   * @return collateral of the HMKT Token
   * @dev HMKT token is 18 decimals
  ...
  {
    uint256 HMKTPrice = HMKTPrice();
    uint256 collateralPrice = getOraclePrice(collateralPriceOracle);
    collateral = ((HMKTPrice.mul(_amount).mul(ratio)).div(100))
      .div(collateralPrice)
      .div(collateralDecimalsAdjustmentFactor);
  }

  /**
   * @notice Returns the minimal required HMKT to liquidate a Vault
   * @param _vaultId of the vault to liquidate
   * @return amount required of the HMKT Token
   * @dev LT = ((((D * r) / 100) - cHMKT) * 100) / (r - (p + 100))
   * cHMKT = ((C * cdaf * cp) / P)
   * LT = Required HMKT
   * D = Vault Debt
   * C = Required Collateral
   ...
   */
  function requiredLiquidationHMKT(uint256 _vaultId)
    public
    view
    virtual
    returns (uint256 amount)
  {
    ...
    uint256 HMKTPrice = HMKTPrice();
    uint256 collateralPrice = getOraclePrice(collateralPriceOracle);
    uint256 collateralHMKT = (
      vault.Collateral.mul(collateralDecimalsAdjustmentFactor).mul(
        collateralPrice
      )
    ).div(HMKTPrice);
    uint256 reqDividend = (
      ((vault.Debt.mul(ratio)).div(100)).sub(collateralHMKT)
    ).mul(100);
  ...
  {
    Vault memory vault = vaults[_vaultId];
    uint256 req = requiredLiquidationHMKT(_vaultId);
    uint256 HMKTPrice = HMKTPrice();
    uint256 collateralPrice = getOraclePrice(collateralPriceOracle);
    uint256 reward = (req.mul(liquidationPenalty.add(100)));
    uint256 _rewardCollateral = (reward.mul(HMKTPrice))
    ...
    returns (uint256 currentRatio)
  {
    ...
    if (vault.Id == 0 || vault.Debt == 0) {
      ...
      currentRatio = (
        ...
        ).div(vault.Debt.mul(HMKTPrice()))
      );
    }
  }

  /**
   * @notice Returns the required fee of ETH to burn the HMKT tokens
  ...
  function getFee(uint256 _amount) public view virtual returns (uint256 fee) {
    ...
    fee = (HMKTPrice().mul(_amount).mul(burnFee)).div(10000).div(ethPrice);
  }

  /**
   * @notice Burns an amount of HMKT Tokens
  ...
  function _burn(uint256 _vaultId, uint256 _amount) internal {
    ...
    HMKTToken.burn(msg.sender, _amount);
  }
}
